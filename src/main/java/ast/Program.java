...
public class Program {
	...
	public void updateFields(Contract cnt) {
		if (cnt.getGlobalVars() == null || fields == null)
			return;
	...
	public void updateParties(Contract cnt) {
		if (cnt.getGlobalParties() == null || parties == null)
			return;
				...
				case 0 : // first execution
					...
					if (this.getFields() != null) {
						...
						for (Field f : this.getFields()) {
							if (f.getType() != null
									&& (f.getType() instanceof TimeType || f.getType() instanceof BooleanType
											|| f.getType() instanceof RealType || f.getType() instanceof StringType)) {
								System.out.print("\t");
								f.printField();
							}
						}
							...
							if (c.getVars() != null) {
								for (int i = 0; i < c.getVars().size(); i++) {
									if (i != c.getVars().size() - 1) {
										System.out.print(c.getVars().get(i).getType().getTypeName() + " "
												+ c.getVars().get(i).getId() + ",");
									} else {
										System.out.print(c.getVars().get(i).getType().getTypeName() + " "
												+ c.getVars().get(i).getId());
									}
							...
							if (c.getAssets() != null) {
								for (int i = 0; i < c.getAssets().size(); i++) {
									if (i != c.getAssets().size() - 1) {
										System.out.print(c.getAssets().get(i).getType().getTypeName() + " "
												+ c.getAssets().get(i).getId() + ",");
									} else {
										System.out.print(c.getAssets().get(i).getType().getTypeName() + " "
												+ c.getAssets().get(i).getId());
									}
				...
				case 3 :
					...
					for (int index = 0; index < this.getContracts().size(); index++) {
						...
						if (rightId && c.getId().equals(nameContract) && rightInitState) {
							...
							boolean emptyVars = varsItems.length == 1 && varsItems[0].length() == 0
									&& tmpContr.getVars() != null && tmpContr.getVars().size() == 0;
							boolean emptyAssets = assetsItems.length == 1 && assetsItems[0].length() == 0
									&& tmpContr.getAssets() != null && tmpContr.getAssets().size() == 0;
							boolean equalDimVars = tmpContr.getVars() != null
									&& varsItems.length == tmpContr.getVars().size() && varsItems[0].length() != 0;
							boolean equalDimAssets = tmpContr.getAssets() != null
									&& assetsItems.length == tmpContr.getAssets().size()
									&& assetsItems[0].length() != 0;
							...
							if (rightDimVars && rightDimAssets) {
								...
								if (tmpContr.getVars() != null) {
									for (int i = 0; i < tmpContr.getVars().size(); i++) {
										DateUtils d = new DateUtils();
										if (d.isValidDate(varsItems[i]) && (!(tmpContr.getVars().get(i)
												.getType() instanceof TimeType)
												|| !(tmpContr.getVars().get(i).getType() instanceof GeneralType))) {
											rightTypes = false;
										} else if ((varsItems[i].equals("true") || varsItems[i].equals("false"))
												&& !(tmpContr.getVars().get(i).getType() instanceof GeneralType)
												&& !(tmpContr.getVars().get(i).getType() instanceof BooleanType)) {
											rightTypes = false;
										} else if (!(tmpContr.getVars().get(i).getType() instanceof GeneralType)
												&& !(tmpContr.getVars().get(i).getType() instanceof RealType)
												&& !(tmpContr.getVars().get(i).getType() instanceof TimeType)
												&& !(tmpContr.getVars().get(i).getType() instanceof StringType)) {
											rightTypes = false;
										}
									}
								}
								if (rightTypes) {
									...
									if (assetsItems[0].length() != 0 && tmpContr.getAssets() != null) {
										for (int i = 0; i < assetsItems.length; i++) {
											...
											newAsset.move((float) Double.parseDouble(assetsItems[i]),
													tmpContr.getAssets().get(i));
											typeinferencer.addType(tmpContr.getAssets().get(i).getId(), new AssetType(),
													index);
