...
public class TypeChecker extends StipulaBaseVisitor<Object> {
	...
	public Map<Pair<String, Integer>, Type> setType(Pair<String, Integer> pair, Type type,
			Map<Pair<String, Integer>, Type> map) {
		for (Pair<String, Integer> s : map.keySet()) {
			if (s.equals(pair)) {
				map.put(pair, type);
			}
	...
	public void addElementsMap(Map<Pair<String, Integer>, Type> toRet) {
		for (Pair<String, Integer> s : toRet.keySet()) {
			...
			if (!isPresent(s, types)) {
				...
				if (tmpType == null || (!(tmpType instanceof RealType) && !(tmpType instanceof BooleanType)
						&& !(tmpType instanceof AssetType) && !(tmpType instanceof TimeType))) {
					for (Entry<Pair<String, Integer>, Type> entry : types.entrySet()) {
						if (entry.getKey().getKey().equals(s.getKey())
								&& (entry.getKey().getValue().equals(s.getValue()) || entry.getKey().getValue() == 0)) {
	...
	public Map<Pair<String, Integer>, Type> visitProg(StipulaParser.ProgContext ctx) {
		...
		for (StipulaParser.FunContext f : ctx.fun()) {
			...
			for (Pair<String, Integer> s : tmp.keySet()) {
				if (!isPresent(s, types)) {
					...
					if (tmpType == null || (!(tmpType instanceof RealType) && !(tmpType instanceof BooleanType)
						...
						for (Entry<Pair<String, Integer>, Type> entry : types.entrySet()) {
							if (entry.getKey().getKey().equals(s.getKey())
									&& (entry.getKey().getValue().equals(s.getValue())
											|| entry.getKey().getValue() == 0)) {
			...
			/*
			 * if(contractNames == null) { contractNames = new ArrayList<String>(); } String
			 * name = ""; for(PartyContext dc : f.party()) { name = name+dc.ID().getText();
	...
	public Map<Pair<String, Integer>, Type> visitStat(StipulaParser.StatContext ctx) {
		...
		if (ctx.ASSETUP() != null) {
			...
			if (typeLeft != null && typeRight != null && !typeLeft.equals(typeRight)) {
				System.out.println("Expressions not of the same type (" + typeLeft.getTypeName() + " and "
						+ typeRight.getTypeName() + ")");
				System.exit(0);
			}
