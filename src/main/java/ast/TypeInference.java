...
public class TypeInference {
	...
	public TypeInference(Map<Pair<String, Integer>, Type> t, ArrayList<String> cn,
			ArrayList<Pair<String, ArrayList<Pair<String, Type>>>> fP) {
		types = t;
		contractNames = cn;
		funParams = fP;
	}
	...
	public boolean isTypeCorrect(Field v1, Field v2, int index) {
		...
		else if (!(t2 instanceof GeneralType) && !(t1 instanceof GeneralType)
				&& !t1.getTypeName().equals(t2.getTypeName())) {
			if ((t1 instanceof AssetType && t2 instanceof RealType)
					|| (t2 instanceof AssetType && t1 instanceof RealType)) {
				correct = true;
			} else {
				correct = false;
			}
	...
	public void addType(String str, Type t, int index) {
		for (Pair<String, Integer> pair : types.keySet()) {
			if ((str.equals(pair.getKey()) && ((pair.getValue() == index + 1) || (pair.getValue() == 0 && index == 0))
					&& t != null)) {
				if (!(types.get(pair) instanceof TimeType)) {
					types.put(pair, t);
				}
