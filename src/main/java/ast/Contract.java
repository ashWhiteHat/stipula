...
public class Contract {
	...
	public Contract(String name, ArrayList<Field> f, ArrayList<Asset> a, ArrayList<Party> d, ArrayList<String> s1,
			String s2, int i) {
		id = name;
		vars = f;
		assets = a;
		...
		if (left != null) {
			...
			if (indexLeft != -1) {
				...
				if (indexLeft != -1) {
					...
					if (indexLeft != -1) {
						if ((vars.get(indexLeft).getType() instanceof TimeType)
								|| vars.get(indexLeft).getValueStr() == null) {
						...
						if (indexLeft != -1) {
							if ((globalVars.get(indexLeft).getType() instanceof TimeType)
									|| globalVars.get(indexLeft).getValueStr() == null) {
		...
		if (right != null) {
			...
			if (indexRight != -1) {
				...
				if (indexRight != -1) {
					...
					if (indexRight != -1) {
						if ((vars.get(indexRight).getType() instanceof TimeType)
								|| vars.get(indexRight).getValueStr() == null) {
						...
						if (indexRight != -1) {
							if ((globalVars.get(indexRight).getType() instanceof TimeType)
									|| globalVars.get(indexRight).getValueStr() == null) {
		...
		for (Statement s : stms) {
			...
			if (s.getOperator().equals("FIELDUP")) {
				...
				if (sourceEntity != null && indexRight != -1) {
					...
					System.err.println("Could not execute FIELDUP statement for source '" + leftExprName
							+ "' and destination '" + rightExpr.getId() + "'");
				}
				...
				if (s.getFract() != 0) {
				...
				} else if (nameFract != null && (nameFract.contains("/") || nameFract.contains("*")
						|| nameFract.contains("+") || nameFract.contains("-"))) {
					...
					if (operands.length == 2) {
						...
						switch (operator) {
							case '/' :
								valFract = (rightVal == 0) ? 0 : leftVal / rightVal;
								break;
							case '*' :
								valFract = leftVal * rightVal;
								break;
							case '+' :
								valFract = leftVal + rightVal;
								break;
					...
					if (indexLeft != -1) {
						if (partyLeft)
							valFract = (float) globalParties.get(indexLeft).getValueAsset();
						else if (globalLeft)
							valFract = (float) globalAssets.get(indexLeft).getValue();
						else
							valFract = (float) assets.get(indexLeft).getValue();
					}
				}
	...
	public boolean runContract(TypeInference tc, int index) {
		...
		if (ifThenElse != null && valid) {
			...
			for (Pair<Expression, ArrayList<Statement>> pair : ifThenElse) {
				if (pair.getKey() != null) {
					if ((pair.getKey().getLeftComplexExpr() == null && pair.getKey().getLeft().getId().equals("_"))
							|| pair.getKey().isValidExpr(pair.getKey())) {
						if (!flag) {
	...
	private Entity evaluateExpression(Expression expr) {
		...
		switch (op) {
			case "*" :
				...
				break;
			case "/" :
				result.setValue(leftValue.getValue() == 0 ? 0 : leftValue.getValue() / rightValue.getValue());
				break;
			case "+" :
				result.setValue(leftValue.getValue() + rightValue.getValue());
				break;
